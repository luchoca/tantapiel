...
const whatsapp = 'https://wa.me/59898700322?text=Hola%2C%20quisiera%20consultar%20por%20un%20tratamiento%20en%20Tanta%20Piel.';
...
function App(){
  ...
  const submitForm = async (event) => {
    ...
    if (response.ok) { form.reset(); setSubmitted(true); }
  };

  return <div className="site">
    <header className="header">
      <button className="brand" onClick={()=>window.scrollTo({top:0,behavior:'smooth'})}>Tanta Piel</button>
      <button className="menu-toggle" aria-expanded={menuOpen} aria-controls="site-nav" onClick={()=>setMenuOpen(!menuOpen)}><span></span><span></span><span></span><b>{menuOpen ? 'Cerrar' : 'Menú'}</b></button>
      <nav id="site-nav" className={menuOpen ? 'nav nav-open' : 'nav'} aria-label="Navegación principal">
        ...
        <button className="book" onClick={()=>navigate('booking')}>Reservar turno <span>↗</span></button>
      </nav>
    </header>
    ...
    <main>
      ...
      <section id="about" className="intro section-pad reveal"><div className="eyebrow">{content.intro.eyebrow}</div><h1>{content.intro.title}<br/><em>{content.intro.emphasis}</em></h1><div className="intro-copy"><p>{content.intro.text}</p><button className="text-link" onClick={()=>go('treatments')}>{content.intro.link} <span>↗</span></button></div></section>
      ...
      <section className="cta section-pad"><div className="eyebrow">{content.cta.eyebrow}</div><h2>{content.cta.title}<br/><em>{content.cta.emphasis}</em></h2><button className="cta-button" onClick={()=>go('booking')}>Reservar turno <span>↗</span></button></section>
    </main>
